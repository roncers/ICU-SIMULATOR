...
simulate <- function(maxT,maxB,maxD) {
  ...
  while (t<maxTime) {
    ...
    if(turnTime >= 96 || t == 0){
      for(i in 1:length(doctors)){
        if(i > actualTurn*turnIndex && i <= (actualTurn+1)*turnIndex) {
          ...
          finalLog[t+1,maxBeds*2 + (i-1)*doctorData + 2] <- 1
          finalLog[t+1,maxBeds*2 + (i-1)*doctorData + 3] <- t*timeUnit
          t <- t + 1
          finalLog[t+1,] <- finalLog[t,]
          turnTime <- turnTime + 1
          ...
          finalLog[t+1,maxBeds*2 + (i-1)*doctorData + 2] <- 0
          finalLog[t+1,maxBeds*2 + (i-1)*doctorData + 3] <- t*timeUnit
          finalLog[t+1,maxBeds*2 + (i-1)*doctorData + 4] <- 0
          finalLog[t+1,maxBeds*2 + (i-1)*doctorData + 5] <- 0
          t <- t + 1
          finalLog[t+1,] <- finalLog[t,]
          turnTime <- turnTime + 1
        }
    ...
    if(!is_empty(busyDoctors)){
      for(doctor in busyDoctors){
        ...
        finalLog[t+1,maxBeds*2 + (doctor-1)*doctorData + 4] <- 0
        finalLog[t+1,maxBeds*2 + (doctor-1)*doctorData + 5] <- 0
      }
    }
    ...
    if(DoctorsBreak()>=1.2 && oneBone){
      ...
      finalLog[t+1,maxBeds*2 + (doctor-1)*doctorData + 5] <- 1
      if(is.na(finalLog[t+1,maxBeds*2 +(doctor-1)*doctorData + 7])){
        finalLog[t+1,maxBeds*2 + (doctor-1)*doctorData + 7] <- 1
      } else {
        finalLog[t+1,maxBeds*2 + (doctor-1)*doctorData + 7] <- finalLog[t+1,maxBeds*2 +(doctor-1)*doctorData + 7] + 1
      }
     ...
     if(!is_empty(waitToEnter)){
       for(patient in waitToEnter){
         aux <- maxBeds*2 + doctorData*length(doctors) + (patient-1)*patientDataL + 3
         if(any(is.na(finalLog[t+1,aux]))){
           finalLog[t+1,aux] <- timeUnit
         } else {
           finalLog[t+1,aux] <- finalLog[t+1,aux] + timeUnit
         }
       }
     }
    ...
    if(oneBone && PatientsFunction()){   #if a patient arrives
      ...
      aux <- maxBeds*2 + doctorData*length(doctors) + (id-1)*patientDataL
      finalLog[t+1,aux  + 4] <- 1
      id <- id + 1
      oneBone <- FALSE
      }
    ...
    if(!is_empty(beds)){
      for(patient in beds){
        aux <- maxBeds*2 + doctorData*length(doctors) + (patient-1)*patientDataL + 2
        if(any(is.na(finalLog[t+1,aux]))){
          finalLog[t+1,aux] <- timeUnit
        } else {
          finalLog[t+1,aux] <- finalLog[t+1,aux] + timeUnit
    ...
    if(oneBone && !is_empty(waitToEnter) && maxBeds > length(beds) && !is_empty(freeDoctors) && evaluateTolerance(DoctorsAttend(),DoctorsTolerance,1)){
      ...
      aux <- maxBeds*2 + doctorData*length(doctors) + (patientID-1)*patientDataL
      finalLog[t+1,aux  + 4] <- 0
      bedID <- min(freeBeds)
      occupiedBeds <- append(occupiedBeds, list(b=bedID,p=patientID))
      finalLog[t+1,(bedID-1)*2+2] <- 1
      finalLog[t+1,aux  + 5] <- bedID
      index <- which(freeBeds == bedID)[1]
      freeBeds <- freeBeds[-index]
      finalLog[t+1,aux  + 6] <- 1
      finalLog[t+1,aux  + 7] <- 0
      finalLog[t+1,maxBeds*2 + (docID-1)*doctorData + 4] <- patientID
      if(is.na(finalLog[t+1,maxBeds*2 +(docID-1)*doctorData + 6])){
        finalLog[t+1,maxBeds*2 + (docID-1)*doctorData + 6] <- 1
      } else {
        finalLog[t+1,maxBeds*2 + (docID-1)*doctorData + 6] <- finalLog[t+1,maxBeds*2 +(docID-1)*doctorData + 6] + 1
      }
      oneBone <- FALSE
    }
      ...
      if(!is_empty(waitToExit)){
        for(patientID in beds) {
          if(oneBone && patientVisit[patientID] > 0 && patientID %in% waitToExit){
            ...
            aux <- maxBeds*2 + doctorData*length(doctors) + (patientID-1)*patientDataL
            finalLog[t+1,aux  + 7] <- finalLog[t+1,aux  + 7] + 1
            oneBone <- FALSE
          }
        }
      }
  ...
  colN <- c()
  for (bed in 1:maxBeds){
    colN <- append(colN,paste("cama ",bed))
    colN <- append(colN,sprintf("c%d hola ocupada", bed))
  }
  for (doctor in doctors){
    colN <- append(colN,paste("doctor ",doctor))
  ...
  for (patient in 1:(id-1)){
    colN <- append(colN,paste("paciente ",patient))
